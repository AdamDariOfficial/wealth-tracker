...
export function InlineEditDialog({
  open, entry, override, accounts, assets, goals,
  onClose, onSave,
}: {
  open: boolean;
  ...
  useEffect(() => { setForm(override); }, [override, entry?.lineNo]);
      ...
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-base">
            Edit row {entry.lineNo} · <span className="capitalize text-muted-foreground">{kind.replace("_", " ")}</span>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3">
          {showAccount && (
            <Field label="Account">
              <Select value={accountId} onChange={(v) => set("accountId", v || null)} options={[
                { v: "", label: "—" },
          ...
          {showTransfer && (
            <div className="grid grid-cols-2 gap-2">
              <Field label="From">
                <Select value={fromId} onChange={(v) => set("fromAccountId", v || null)} options={[
                  { v: "", label: "—" },
                  ...accounts.map((a) => ({ v: a.id, label: a.name })),
                ]} />
              </Field>
              <Field label="To">
                <Select value={toId} onChange={(v) => set("toAccountId", v || null)} options={[
                  { v: "", label: "—" },
                  ...accounts.map((a) => ({ v: a.id, label: a.name })),
                ]} />
              </Field>
          ...
          {showAsset && (
            <Field label="Asset">
              <Select value={assetId} onChange={(v) => set("assetId", v || null)} options={[
                { v: "", label: "—" },
          ...
          {showGoal && (
            <Field label="Goal">
              <Select value={goalId} onChange={(v) => set("goalId", v || null)} options={[
                { v: "", label: "—" },
                ...goals.map((g) => ({ v: g.id, label: g.name })),
              ]} />
            </Field>
          )}
          ...
          <div className="grid grid-cols-2 gap-2">
            {showAmount && (
              <Field label="Amount">
                <Input type="number" inputMode="decimal" value={form.amount ?? entry.amount}
                  onChange={(e) => set("amount", Number(e.target.value))} />
              </Field>
            )}
            {showTarget && (
              <Field label="Target">
                <Input type="number" inputMode="decimal" value={form.targetAmount ?? entry.targetAmount ?? 0}
                  onChange={(e) => set("targetAmount", Number(e.target.value))} />
              </Field>
            )}
            {showQtyPrice && (
              <Field label="Quantity">
                <Input type="number" inputMode="decimal" value={form.quantity ?? entry.quantity ?? 0}
                  onChange={(e) => set("quantity", Number(e.target.value))} />
              </Field>
            )}
            {showQtyPrice && (
              <Field label="Price">
                <Input type="number" inputMode="decimal" value={form.price ?? entry.price ?? 0}
            ...
            <Field label="Category">
              <Input value={form.category ?? entry.category ?? ""} onChange={(e) => set("category", e.target.value || null)} />
            </Field>
          </div>

          <Field label="Description">
            <Textarea value={form.description ?? entry.description ?? ""}
