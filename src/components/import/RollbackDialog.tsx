...
import { computeRollbackImpact, executeRollback, type RollbackScope, type RollbackImpact } from "@/lib/import-rollback";
import { formatMoney } from "@/lib/format-currency";
import { cn } from "@/lib/utils";

const MODES: { key: RollbackScope["mode"]; label: string; hint: string }[] = [
  { key: "all",          label: "Entire batch",           hint: "Void every transaction and archive every safe entity this batch created." },
  { key: "transactions", label: "Only transactions",      hint: "Void ledger rows but keep created assets, goals and accounts." },
  { key: "entities",     label: "Only created entities",  hint: "Archive assets/goals/accounts created by this batch. Keeps transactions." },
  { key: "kinds",        label: "By operation kind",      hint: "Void only rows of the operations you select." },
  { key: "rows",         label: "Selected rows",          hint: "Void only the specific rows you already picked." },
];

const KINDS = ["deposit","expense","transfer","buy","sell","goal_contribution","goal_create","account_open","asset_open"] as const;
...
export function RollbackDialog(props: {
  ...
  async function confirm() {
    ...
    try {
      ...
      toast.success(`Rollback complete · ${r.voidedTx} tx voided · ${r.archivedAssets + r.archivedGoals + r.archivedAccounts} entities archived`);
      onDone?.();
      onClose();
    } catch (e: any) {
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-4">
              ...
              return (
                <button key={m.key} disabled={disabled}
                  ...
                  className={cn(
                    ...
                    mode === m.key ? "border-cyan/50 bg-cyan/5" : "border-border/40 hover:border-border/70",
          ...
          {mode === "kinds" && (
            <div className="flex flex-wrap gap-1.5">
              {KINDS.map((k) => {
                ...
                return (
                  <button key={k} onClick={() => setKinds((p) => {
                    const n = new Set(p); active ? n.delete(k) : n.add(k); return n;
                  })}
                    className={cn(
                      "h-7 px-2 rounded-md text-[11px] border",
                      active ? "border-cyan/40 bg-cyan/10 text-cyan" : "border-border/40 text-muted-foreground",
                    )}>
                    {k.replace("_", " ")}
                  </button>
                );
            ...
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  ...
                  <Stat label="Transfer pairs" value={String(impact.transferGroupsAffected.length)} />
                  <Stat label="Goal reversals" value={String(impact.goalReversals.length)} />
                  <Stat label="Net-worth Δ" value={formatMoney(-impact.netWorthDelta, { currency: ccy })} tone={impact.netWorthDelta > 0 ? "destructive" : impact.netWorthDelta < 0 ? "success" : "muted"} />
                </div>
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <Stat label="Accounts archived" value={String(impact.archivableAccountIds.length)} />
                </div>
                {impact.blockedEntities.length > 0 && (
                  <div className="rounded-md border border-warning/30 bg-warning/5 p-2 space-y-1">
                    <div className="flex items-center gap-1.5 text-warning font-medium">
                      <ShieldAlert className="h-3.5 w-3.5" /> Cannot archive {impact.blockedEntities.length} entit{impact.blockedEntities.length === 1 ? "y" : "ies"}
                    </div>
                    {impact.blockedEntities.slice(0, 6).map((b, i) => (
                      <div key={i} className="text-[11px] text-muted-foreground">· <span className="text-foreground">{b.kind}</span> — {b.reason}</div>
                    ))}
                  </div>
                )}
                {nothingToDo && (
                  <div className="text-[11px] text-muted-foreground italic">Nothing to reverse for this scope.</div>
                )}
              </>
            ) : null}
          </div>
          ...
          <div className="text-[11px] text-muted-foreground flex items-start gap-1.5">
            ...
            Financial history is preserved. Transactions are soft-voided (<code>voided_at</code>) and entities are archived. Nothing is hard-deleted.
          </div>
        </div>

        <DialogFooter>
          <Button variant="ghost" onClick={onClose} disabled={busy}>Cancel</Button>
          <Button onClick={confirm} disabled={busy || loading || nothingToDo} className="bg-warning text-background hover:bg-warning/90">
