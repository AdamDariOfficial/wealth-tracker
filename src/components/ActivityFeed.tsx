...
function Row({ e, onClick }: { e: ActivityEvent; onClick: () => void }) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "h-8 w-8 rounded-lg glass-strong flex items-center justify-center shrink-0",
          tone,
        )}
      >
...
export function ActivityFeed({
  ...
  if (!events.length) {
    return (
      <div className="text-center text-xs text-muted-foreground py-6">{t("No activity yet.")}</div>
    );
  }
  ...
  return (
    <>
      <div className="glass rounded-2xl overflow-hidden divide-y divide-border/30">
        {shown.map((e) => (
          <Row key={e.id} e={e} onClick={() => setSelected(e)} />
        ))}
        {compact && events.length > (limit ?? 6) && (
          ...
          >
            ...
            {expanded ? t("Collapse") : `${t("Show more")} (${events.length - (limit ?? 6)})`}
          </button>
        )}
      </div>
