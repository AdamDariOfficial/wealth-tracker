...
import type { AccountValuePointView } from "@/application/view-models";
...
export function AccountValueTrendChart({
  ...
  points: readonly AccountValuePointView[];
  locale?: string;
}) {
  ...
  const data = useMemo(
    () =>
      points.map((point) => ({
        ...
        display: formatMoney(point.knownValue, locale),
        complete: point.complete,
        value: Number(point.knownValue.amount.toString()),
      })),
      ...
      <AreaChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
        <defs>
          <linearGradient id="accountValueFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="var(--color-cyan)" stopOpacity={0.24} />
              ...
              <div style={chartTooltipProps.contentStyle}>
                ...
                {!point.complete && (
                  <div className="mt-1 text-[11px] text-warning">
                    {tr(normalizeAppLocale(locale), "Some asset values were unavailable")}
                  </div>
                )}
              </div>
            );
