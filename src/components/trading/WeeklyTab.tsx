...
export function WeeklyTab() {
  ...
  const commit = async () => {
    try {
      ...
      toast.success(
        posted_transaction_id
          ? "Review saved · P&L posted · broker balance reconciled"
          : "Weekly review saved",
      );
  ...
  const removeReport = async (r: WeeklyReport) => {
    if (r.finalized_at) {
      toast.error("Report is finalized. Unlock it first.");
      return;
    }
  ...
  const equityCurve = weekly
    .slice()
    .reverse()
    .reduce<{ idx: number; equity: number; week: string }[]>((acc, r, i) => {
  ...
  return (
    <div className="space-y-6">
        ...
        {[
          {
            l: "Last P&L",
            v: weekly[0]
              ? `${Number(weekly[0].pnl) >= 0 ? "+" : ""}$${Number(weekly[0].pnl).toFixed(2)}`
              : "—",
              ...
              <tr key={r.id} className="border-b border-border/30 hover:bg-muted/20">
                ...
                <td
                  className={cn(
                    ...
                    Number(r.pnl) >= 0 ? "text-success" : "text-destructive",
                  )}
                >
                ...
                <td className="px-4 py-3 font-mono text-cyan font-semibold">
                  {r.consistency_score}
                </td>
                ...
                <td className="px-4 py-3">
                  <Button
                    variant="ghost"
                    size="icon"
                    disabled={!!r.finalized_at}
                    onClick={() => removeReport(r)}
                  >
                    <Trash2
                      className={cn(
                        "h-3.5 w-3.5",
                        r.finalized_at ? "text-muted-foreground/40" : "text-destructive",
                      )}
                    />
                  </Button>
                </td>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button className="bg-cyan text-background hover:bg-cyan/90" onClick={openConfirm}>
              {willPost ? "Review & post" : "Save review"}
            </Button>
          </>
        }
      >
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setForm({ ...form, num_trades: +e.target.value })}
              className="mt-1"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <Label className="text-xs">Avg R:R</Label>
            <Input
              type="number"
              step="0.01"
              value={form.avg_rr || ""}
              onChange={(e) => setForm({ ...form, avg_rr: +e.target.value })}
              className="mt-1"
            />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-xs">Discipline (0–100)</Label>
            <Input
              type="number"
              min={0}
              max={100}
          ...
          <div>
            <Label className="text-xs">Psychology (0–100)</Label>
            <Input
              type="number"
              min={0}
              max={100}
              value={form.psychology_score}
              onChange={(e) => setForm({ ...form, psychology_score: +e.target.value })}
              className="mt-1"
            />
          </div>
        </div>
        ...
        <AccountPicker
          ...
          onChange={setBrokerAccountId}
          label="Post P&L to broker account (optional)"
      ...
      <Modal
        ...
        title="Confirm weekly P&L posting"
        footer={
          <>
            <Button variant="outline" onClick={() => setConfirmOpen(false)}>
              Back
            </Button>
            <Button className="bg-cyan text-background hover:bg-cyan/90" onClick={commit}>
              Confirm & post
            </Button>
          </>
        }
      >
        ...
        <div className="glass-strong rounded-xl p-4 text-xs font-mono space-y-2 mt-2">
          ...
          <div className="flex justify-between">
            ...
            <span>{accounts.find((a) => a.id === brokerAccountId)?.name ?? brokerAccountId}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span>{form.week_start}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Note</span>
            <span>Weekly P&L · {form.week_start}</span>
          </div>
        </div>
      </Modal>
    </div>
