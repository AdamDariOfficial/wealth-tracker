...
export function OverviewTab() {
  ...
  const stats: { l: string; v: string; c?: string }[] = [
    {
      l: "Realized P&L",
      v: `${metrics.realizedPnl >= 0 ? "+" : ""}$${metrics.realizedPnl.toLocaleString(undefined, { maximumFractionDigits: 2 })}`,
      c: metrics.realizedPnl >= 0 ? "text-success" : "text-destructive",
    },
    {
      l: "% Return on Capital",
      v: `${metrics.pctReturn.toFixed(2)}%`,
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="surface-section p-6 relative overflow-hidden lg:col-span-2"
        >
          ...
          <div className="relative">
            ...
            >
              ...
              {metrics.realizedPnl >= 0 ? "+" : ""}$
              {metrics.realizedPnl.toLocaleString(undefined, { maximumFractionDigits: 2 })}{" "}
              performance · net capital $
              {metrics.netCapital.toLocaleString(undefined, { maximumFractionDigits: 2 })}
            </div>
        ...
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="surface-section p-6 relative overflow-hidden"
        >
          <div className="absolute inset-0 bg-[var(--gradient-glow)] pointer-events-none" />
          <div className="relative">
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              Aggregate of discipline, psychology, drawdown stability and execution across{" "}
              {metrics.weeklyCount} reviews.
            </div>
          </div>
        </motion.div>
      </div>
      ...
      >
        <h3 className="font-display font-semibold flex items-center gap-2">
          <TargetIcon className="h-4 w-4 text-cyan" /> Capital Phases
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          {phases.map((p, idx) => {
            const pct = Math.min(100, (metrics.currentCapital / p.target) * 100);
            const active =
              metrics.currentCapital < p.target &&
              (idx === 0 || metrics.currentCapital >= phases[idx - 1].target);
            return (
              <div
                key={p.name}
                className={cn("surface-quiet p-4", active && "ring-1 ring-cyan/40")}
              >
                ...
                <div className="mt-3 h-2 bg-muted rounded-full overflow-hidden">
                  <motion.div
                    ...
                    style={{ background: `var(--chart-${idx + 1})` }}
                  />
                </div>
